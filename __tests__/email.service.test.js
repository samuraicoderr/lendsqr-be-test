...
describe("email.service", () => {
    it("throws when ZeptoMail api key is missing", () => {
        const { createEmailClient } = loadEmailService({
            ZEPTOMAIL_API_KEY: "",
            ZEPTOMAIL_FROM_ADDRESS: env_1.env.zeptomail.fromAddress,
            ZEPTOMAIL_FROM_NAME: env_1.env.zeptomail.fromName,
    ...
    it("throws when from address or name is missing", () => {
        const { createEmailClient } = loadEmailService({
            ZEPTOMAIL_API_KEY: env_1.env.zeptomail.apiKey,
